...
def evaluateScore_Levenshtein(word,ger,por,pol):
    score = 0
    
    for lang in ger,por,pol:
        score += ld(word,lang)
    
    return score


def evaluateScore_AlloWithVowels(word,ger,por,pol):
    score = 0
    scoreLangs = [0,0,0,0]
    
    leastEfficientWord = ger+por+pol
...
def evaluateScore_ConsonantsInOrder(word,ger,por,pol):
    score = 0
...
for line in data:
    if ',' in line:
        ...
        originalWords = [words['Ger'], words['Por'], words['Pol']]
        leastEfficientWord = words['Ger'] + words['Por'] + words['Pol']
        print '\n'
        print newWord.upper() + ' vs. "' + leastEfficientWord + '":'
        print originalWords
        score1 = evaluateScore_AlloWithVowels(newWord, words['Ger'], words['Por'], words['Pol'])
        print '  ' + str(score1) + '\t<- evaluateScore_AlloWithVowels'
        score2 = evaluateScore_ConsonantsInOrder(newWord, words['Ger'], words['Por'], words['Pol'])
        print '  ' + str(score2) + '\t<- evaluateScore_ConsonantsInOrder'
        avgScore = (score1 + score2)/2
        print 'Average score: ' + str(avgScore)
        score3 = evaluateScore_Levenshtein(newWord, words['Ger'], words['Por'], words['Pol'])
