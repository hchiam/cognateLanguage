...
def evaluateScore_Levenshtein(word,mal,kor,swa,jap):
    score = 0
    
    for lang in mal,kor,swa,jap:
        score += ld(word,lang)
    
    return score


def evaluateScore_AlloWithVowels(word,mal,kor,swa,jap):
    score = 0
    scoreLangs = [0,0,0,0]
    
    leastEfficientWord = mal+kor+swa+jap
...
def evaluateScore_ConsonantsInOrder(word,mal,kor,swa,jap):
    score = 0
...
for line in data:
    if ',' in line:
        ...
        leastEfficientWord = words['Mal'] + words['Kor'] + words['Swa'] + words['Japa']
        print '\n'
        print newWord.upper() + ' vs. "' + leastEfficientWord + '":'
        print originalWords
        score1 = evaluateScore_AlloWithVowels(newWord, words['Mal'],words['Kor'],words['Swa'],words['Japa'])
        print '  ' + str(score1) + '\t<- evaluateScore_AlloWithVowels'
        score2 = evaluateScore_ConsonantsInOrder(newWord, words['Mal'],words['Kor'],words['Swa'],words['Japa'])
        print '  ' + str(score2) + '\t<- evaluateScore_ConsonantsInOrder'
        avgScore = (score1 + score2)/2
        print 'Average score: ' + str(avgScore)
        score3 = evaluateScore_Levenshtein(newWord, words['Mal'],words['Kor'],words['Swa'],words['Japa'])
